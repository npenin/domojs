...
export function sub<T extends ProtobufMessage<T>>(parser: AnyParser<T, Partial<T>>)
    : ParserWithMessageWithoutKnownLength<T | null, any> & { wireType: WireType }
{
...
export function property<T extends ProtobufMessage<T>, TKey extends keyof T>(name: TKey, wireType: WireType, parser: AnyParser<T[TKey] | null, T>)
    : ParserWithMessageWithoutKnownLength<T[TKey] | null, T> & { wireType: WireType }
export function property<T extends ProtobufMessage<T>, TKey extends keyof T>(name: TKey, parser: (ParsersWithMessage<T[TKey] | null, Partial<T>> & { wireType: WireType }))
    : ParserWithMessageWithoutKnownLength<T[TKey] | null, T> & { wireType: WireType }
export function property<T extends ProtobufMessage<T>, TKey extends keyof T>(name: TKey, wireType: WireType | (ParsersWithMessage<any, Partial<T>> & { wireType: WireType }), parser?: AnyParser<T[TKey] | null, Partial<T>>)
    : ParserWithMessageWithoutKnownLength<T[TKey] | null, T> & { wireType: WireType }
{
    ...
    return new Property<T, typeof name>(name, wireType, new ZeroOrOne(parser as AnyParser<T[TKey] | null, Partial<T>>) as any) as any;
}
...
export function message<T extends ProtobufMessage<T>>(...parsers: (ParserWithMessageWithoutKnownLength<any, Partial<T>> & { wireType: WireType })[]):
    ParserWithMessageWithoutKnownLength<T | null, Partial<T> | void> & { wireType: WireType }
{
