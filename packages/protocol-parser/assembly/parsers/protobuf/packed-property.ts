...
export default class PackedProperty<T, TKey extends keyof T> implements ParserWithMessage<ArrayItem<T[TKey]>[] | ArrayItem<T[TKey]>, T>
{
    ...
    read(buffer: IBuffer, cursor: Cursor, message: T): ArrayItem<T[TKey]>[] | ArrayItem<T[TKey]>
    {
    ...
    write(value: ArrayItem<T[TKey]>[] | ArrayItem<T[TKey]>, message: T): IBuffer[]
    write(buffer: IBuffer, cursor: Cursor, value: ArrayItem<T[TKey]>[] | ArrayItem<T[TKey]>, message: T): void
    write(buffer: IBuffer | ArrayItem<T[TKey]>[] | ArrayItem<T[TKey]>, cursor?: Cursor | T, value?: ArrayItem<T[TKey]>[] | ArrayItem<T[TKey]>, message?: T)
    {
        ...
        if (message)
        {
            const value = message[this.name] as ArrayItem<T[TKey]>[] | ArrayItem<T[TKey]>;
            if (Array.isArray(value))
                for (var v in value)
                    multiPush(buffers, parserWrite(this.parser, value[v], value[v]));
