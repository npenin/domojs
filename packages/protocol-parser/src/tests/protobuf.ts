...
import { Buffer } from "../../biuld/core";
import { protobuf } from "../../build/parsers"
import { ProtobufMessage } from "../../build/parsers/protobuf";
...
describe('protobuf', function ()
{
    ...
    it('should parse varint', function ()
    {
        ...
        assert.deepStrictEqual(Buffer.of(0b10101100, 0b00000010), buffer);
    ...
    it('should parse object with varint', function ()
    {
        ...
        assert.deepStrictEqual(Buffer.of(0x08, 0x96, 0x01), buffer);
    ...
    it('should parse object with string', function ()
    {
        ...
        var buffer = Buffer.concat(parserWrite(message, expected, expected));
        assert.deepStrictEqual(Buffer.of(0x12, 0x07, 0x74, 0x65, 0x73, 0x74, 0x69, 0x6e, 0x67), buffer);
        assert.deepStrictEqual(expected, message.read(buffer, new Cursor(), {}));
    })
    it('should parse nested object', function ()
    {
        ...
        var buffer = Buffer.concat(parserWrite(message, expected, expected));
        assert.deepStrictEqual(Buffer.of(0x1a, 0x03, 0x08, 0x96, 0x01), buffer);
        assert.deepStrictEqual(expected, message.read(buffer, new Cursor(), {}));
    })
})