...
FROM alpine
RUN apk --no-cache --update add yarn npm coreutils eudev-libs && \
    ...
    echo 'SUBSYSTEM=="usb", ATTRS{idVendor}=="*", ATTRS{idProduct}=="*", MODE="0660",GROUP="dialout"' > /etc/udev/rules.d/99-usb.rules
